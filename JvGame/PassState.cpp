...
// 	add(map);
	...
	txt = new JvText(210,140,200,50,FONT_NAME,"Pass",20);
	//txt->setSize(20);
	...
	nextButton->setTitle("NEXT LEVEL",FONT_NAME,20,6,12);
	...
	exitButton->setTitle("MAIN MENU",FONT_NAME,22,6,12);
	...
	gameoverTile = new JvText(0,10,300,30,FONT_NAME,"Level Complete",20);
...
void PassState::update()
{
	...
	if (exitButton->visible==true) {
		if (JvG::joystick->isJustPreess(UPCODE))
		{
			...
			{
				_curSelectIndex = _maxSelectIndex;
			}
			updateMenu();
		}
		...
		{
			_curSelectIndex++;
			if (_curSelectIndex > _maxSelectIndex)
			{
				_curSelectIndex = 0;
			}
			updateMenu();
		...
		else if (JvG::joystick->isJustPreess(ACODE))
		{
			if (_curSelectIndex == 0)
			{
				::NextButton_callback_ps(0);
			}
			else if (_curSelectIndex == 1)
			{
				::Exit_callback_ps(0);
			}
			updateMenu();
...
void PassState::updateMenu()
{
	if (_curSelectIndex == 0)
	{
		nextButton->selectMode();
		exitButton->normalMode();
	}
	else
	{
		nextButton->normalMode();
		exitButton->selectMode();
	}
}
