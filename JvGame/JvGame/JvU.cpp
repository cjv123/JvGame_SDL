...
SDL_Texture* JvU::loadTexture(const string& imgFilename)
{
	//The final texture
	SDL_Texture* newTexture = NULL;

	//Load image at specified path
	SDL_Surface* loadedSurface = IMG_Load(imgFilename.c_str());
	...
	else
	{
		...
		newTexture = SDL_CreateTextureFromSurface(JvG::jvGameP->getSDLRenderer(),loadedSurface);
		if (newTexture == NULL)
		{
			printf("Unable to create texture from %s! SDL Error: %s\n", imgFilename.c_str(), SDL_GetError());
		}
